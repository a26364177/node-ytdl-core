...
exports.getBasicInfo = (id, options, callback) => {
  ...
  fetch(url, reqOptions)
    ...
    .then((res) => {
      ...
      const unavailableMsg = util.between(res, '<div id="player-unavailable"', '>');
      if (unavailableMsg &&
          !/\bhid\b/.test(util.between(unavailableMsg, 'class="', '"'))) {
        // Ignore error about age restriction.
        if (!res.includes('<div id="watch7-player-age-gate-content"')) {
          throw Error(util.between(res, '<h1 id="unavailable-message" class="message">', '</h1>').trim())
        }
      }
      ...
      const additional = {
        ...
        author: extras.getAuthor(res),

        // Get the day the vid was published.
        published: extras.getPublished(res),

        // Get description.
        description: extras.getVideoDescription(res),

        // Get media info.
        media: extras.getVideoMedia(res),

        // Get related videos.
        related_videos: extras.getRelatedVideos(res),
      };
      const jsonStr = util.between(res, 'ytplayer.config = ', '</script>');
      let config;
      if (jsonStr) {
        ...
        fetch(url, options.requestOptions).then((res) => {
          return res.text()
        }).then((res) => {
